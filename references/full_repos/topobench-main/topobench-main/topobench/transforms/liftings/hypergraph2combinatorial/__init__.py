...
from .base import Hypergraph2CombinatorialLifting


class ModuleExportsManager:
    """Manages automatic discovery and registration of Hypergraph2Combinatorial lifting classes."""

    @staticmethod
    def is_lifting_class(obj: Any) -> bool:
        """Check if an object is a valid Hypergraph2Combinatorial lifting class.
        ...
        -------
        bool
            True if the object is a valid Hypergraph2Combinatorial lifting class (non-private class
            inheriting from Hypergraph2CombinatorialLifting), False otherwise.
        """
        return (
            inspect.isclass(obj)
            and obj.__module__ == "__main__"
            and not obj.__name__.startswith("_")
            and issubclass(obj, Hypergraph2CombinatorialLifting)
            and obj != Hypergraph2CombinatorialLifting
        )

    @classmethod
    def discover_liftings(cls, package_path: str) -> dict[str, type]:
        """Dynamically discover all Hypergraph2Combinatorial lifting classes in the package.
...
HYPERGRAPH2COMBINATORIAL_LIFTINGS = manager.discover_liftings(__file__)

# Automatically generate __all__
__all__ = [
    *HYPERGRAPH2COMBINATORIAL_LIFTINGS.keys(),
    "Hypergraph2CombinatorialLifting",
    "HYPERGRAPH2COMBINATORIAL_LIFTINGS",
]

# For backwards compatibility, create individual imports
locals().update(**HYPERGRAPH2COMBINATORIAL_LIFTINGS)
