...
from ragraph_utils import ToyGraphBase, Propagation, FewShotBase, TaskDecoder, ring_contrastive_loss
from torch_geometric.nn import global_mean_pool


class RAGraph(nn.Module):
    def __init__(self, pretrain_model, resource_dataset, feture_size, num_class, emb_size,
                 finetune=True, noise_finetune=False, dataset_name=None) -> None:
    ...
    def forward(self, features, adj, complex_batch=None, batch=None, ptr=None, return_ring_loss=False):
        ...
        if batch is not None:
            ...
            for i in range(num_graphs):
                ...
                if complex_obj is None or \
                    ...
                    # 如果需要梯度，稍微给一点点值避免断连，但在 eval 模式下通常不需要
                    if self.training:
                    ...
                    single_loss, ring_mean_i = ring_contrastive_loss(
                        ...
                        complex_obj.cochains[1].boundary_index.to(node_emb.device)
                    )
            ...
            if complex_batch is not None and \
                ...
                single_loss, ring_mean = ring_contrastive_loss(
                    ...
                    complex_batch.cochains[1].boundary_index.to(node_emb.device)
                )
                ring_loss = single_loss
        ...
        # === 修复点：防止在 eval 模式下对不需要梯度的 Tensor 调用 retain_grad ===
        ...
        if self.finetune:
            ...
            # decode_label = torch.softmax(decode_label, dim=1) # 通常 logits 不做 softmax 再输出，除非 loss 也是 NLLLoss

            # 混合 logits 和 rag_label (注意：rag_label 通常是 one-hot)
            # 这里简单处理，如果 rag_label 是概率分布，可以混合
    ...
    def forward_with_loss(self, features, adj, complex_batch, label, batch=None, ptr=None):
        ...
        if batch is not None:
            ...
            for i in range(num_graphs):
                ...
                if complex_obj is None or \
                        ...
                        complex_obj.cochains[2].boundary_index is None or \
                        complex_obj.cochains[2].boundary_index.size(1) == 0:
                    # 即使是空，也给一个零向量并允许梯度（虽然梯度是0），保持图计算图完整
                ...
                ring_loss += single_loss
                ring_means.append(ring_mean_i)
                ring_loss = ring_loss / num_graphs
            ring_mean = torch.stack(ring_means, dim=0)
        else:
            # 单图逻辑简化
            single_loss, ring_mean = ring_contrastive_loss(
        ...
        # 可以取消注释来混合 ring loss
        # total_loss = cls_loss + 0.1 * ring_loss
        total_loss = cls_loss

        return total_loss, decode_logits, {
            "cls_loss": cls_loss,
            "ring_loss": ring_loss
        }

    # === 以下是补充的方法，已经移出 forward_with_loss 外部 ===

    def get_graph_and_ring_embeddings(self, features, adj, complex_batch, batch=None, ptr=None):
        """
        Only used for visualization:
        返回 graph-level embedding, ring_feat, rag_embedding.
        不计算 loss，不做反向传播。
        """
        self.eval()
        ...
        with torch.no_grad():
            ...
            if batch is not None:
                ...
                for i in range(num_graphs):
                    ...
                    if (complex_obj is None or
                        ...
                        _, ring_mean_i = ring_contrastive_loss(
                            ...
                            complex_obj.cochains[1].boundary_index.to(device)
                        )
                    ring_means.append(ring_mean_i)
                ring_mean = torch.stack(ring_means, dim=0)
            else:
                _, ring_mean = ring_contrastive_loss(
                    ...
                    complex_batch.cochains[1].boundary_index.to(device)
                )
                ring_mean = ring_mean.unsqueeze(0)
    ...
    def get_graph_embedding_noring(self, features, adj, batch=None, ptr=None):
        """
        只用 GNN graph embedding + RAG 检索，不显式使用 ring 特征。
        """
        self.eval()
        with torch.no_grad():
            node_emb, _ = self.pretrain_model.embed(features, adj)
    ...
    def encode_graph(self, features, adj, complex_batch=None, batch=None):
        """
        适配 fewshot_finetune.py 中的 extract_graph_embeddings 调用。
        返回 (node_emb, graph_emb) 用于 t-SNE 绘图。
        """
        self.eval()
        with torch.no_grad():
            node_emb, _ = self.pretrain_model.embed(features, adj)
            ...
            # 注意：如果你的 t-SNE 需要包含 RAG 信息后的增强 Embedding，
            # 你应该在这里复现 forward 中的逻辑并返回 hidden_embedding。
            # 目前这里只返回基础的 GNN graph embedding。
