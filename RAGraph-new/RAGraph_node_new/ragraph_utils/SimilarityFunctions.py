...
class SimilarityFunctions:
    ...
    def calculate_cosine_similarity(search_keys: torch.Tensor, resource_keys: torch.Tensor) -> float:
        search_keys = torch.nan_to_num(search_keys, nan=0.0, posinf=0.0, neginf=0.0)
        resource_keys = torch.nan_to_num(resource_keys, nan=0.0, posinf=0.0, neginf=0.0)
        # Normalize search_keys (query_num, emb_size)
        search_keys = F.normalize(search_keys, p=2, dim=-1, eps=1e-12)

        # Normalize resource_keys (resource_num, emb_size)
        resource_keys = F.normalize(resource_keys, p=2, dim=-1, eps=1e-12)
        ...
        return torch.nan_to_num(cosine_similarities, nan=0.0, posinf=0.0, neginf=0.0)
